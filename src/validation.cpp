...
const std::string strMessageMagic = "SibCoin Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    /// SIBCOIN: Check superblock start
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SIBCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SIBCOIN): couldn't find masternode or superblock payments"),
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // SIB specific
        if (sporkManager.IsSporkActive(SPORK_20_REJECT_OLD_BLOCKS)) {
            if ( chainActive.Height() - pindexPrev->nHeight > SPORK_21_REJECT_NUMBER) {
