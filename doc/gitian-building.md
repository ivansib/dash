...
*Setup instructions for a Gitian build of Sibcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sibcoin
...
- [Building Sibcoin Core](#building-sibcoin-core)
...
Clone the git repositories for Sibcoin Core and Gitian.
...
in the Sibcoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Sibcoin Core
----------------

To build Sibcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Sibcoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sibcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ivansib/sibcoin
...
For example:
```bash
URL=https://github.com/TriKriSta/sibcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit sibcoin=${COMMIT} --url sibcoin=${URL} ../sibcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sibcoin=${COMMIT} --url sibcoin=${URL} ../sibcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sibcoin=${COMMIT} --url sibcoin=${URL} ../sibcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sibcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sibcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sibcoin
SIGPATH=/some/root/path/sibcoin-detached-sigs

./bin/gbuild --url sibcoin=${BTCPATH},signature=${SIGPATH} ../sibcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sibcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sibcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sibcoin-osx-build.assert
```
...
[dash/gitian.sigs](https://github.com/ivansib/gitian.sigs/) repository, or if that's not possible to create a pull
request.
